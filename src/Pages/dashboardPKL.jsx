...
export const DashboardPKLPage = () => {
  ...
  const [pieChartPKL, setPieChartPKL] = useState({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost/php/home/search-pkl.php?search=${encodeURIComponent(searchTerm)}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost/php/Dashboard/count/countPKL.php');
        if (!response.ok) {
  ...
  useEffect(() => {
    fetch("http://localhost/php/Dashboard/PieChart/PieChartPKL.php")
  ...
  const groupedDataPie = Object.entries(pieChartPKL).map(([nama, jumlah]) => ({
    nama,
    jumlah,
  }));
  ...
  useEffect(() => {
    fetch("http://localhost/php/Dashboard/BarChart/BarChartPKL.php")
      .then((response) => response.json())
      .then((data) => {
    ...
    <div className="flex w-full min-h-screen">
      ...
      >
        ...
        <div className="flex flex-col flex-grow">
          ...
          {/* Bar Chart Start */}
          <div className="my-4">
            <h1 className="bg-sky-800 rounded-lg md:w-1/4 text-center py-2 text-slate-200 font-poppins font-bold md:text-xl 2xl:text-3xl uppercase md:ml-2 2xl:ml-8 md:my-3 2xl:my-5">
              Judul Laporan PKL
            </h1>
            {barChartRendered && (
              <div
