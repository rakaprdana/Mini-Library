...
import { DwnldSkripsi } from "../components/Elements/DwnldSkripsi";

export const DetailSkripsi = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost/php/detail/detailskripsi.php?search=${encodeURIComponent(
            searchParam
          )}`
        );
  ...
  // Fungsi untuk menangani unduhan file
  const handleDownload = (fileUrl) => {
    const anchor = document.createElement("a");
    anchor.href = fileUrl;
    anchor.download = "nama_file_yang_diinginkan";
    anchor.click();
  };
  return (
    <div
    ...
    >
      ...
      <div className="w-full flex flex-grow justify-center items-center">
        <div className="bg-gray-200 w-full mx-1 md:mx-2 md:my-3 2xl:my-0 lg:mx-0 lg:w-4/5 2xl:w-3/5 rounded-t-lg rounded-b-md">
          {selectedItem && (
            <div
          ...
          <div className="flex justify-evenly my-2 md:my-6 2xl:my-10">
            <DwnldSkripsi selectedItem={selectedItem} source={source} />
          </div>
          {/* File Calling End */}
        </div>
      </div>
      <footer className="py-1 md:py-2 2xl:py-4 text-xs md:text-sm lg:text-base bg-customBlue text-center text-slate-200">
        <p className="">
          <span className="font-bold">©</span> Copyright MiniLib Teams. All
          Rights Reverved
        </p>
        <p className="">
          Designed by{" "}
